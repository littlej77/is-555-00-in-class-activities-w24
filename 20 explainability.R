...
#because of the predicted values and the residuals. the data being passed in is wrong. its supposed to be cars_rec and not the training. 
...
#the error says "could not find function "model_profile"" so maybe that function requires predicted values and residuals to run.
...
#they display how the feature interacts with the prediction variable. 
# Why do they look so different? they are different models and have different algorithm to examine the features relationship with the predicting variable. 
# What does this tell you about why xgboost has better performance than linear regression?, xgboost has more freedom to move around whereas the linear has to stay linear. 
...
# And another method:
# Why do these variable importance plots look so different? the scaling is very different. this could possibly have something to do with the exponential curve? we are not sure. 
# Again, what does that tell you about the fundamental differences between xgboost and linear regression? xgboost weighs column heavier than linear regression
